...
{
    public class HousePortionController : ApiController
    {
        [Route("api/house_portion")]
        public IEnumerable<MobileItem> Get()
        {
            MobileItem[] m;
            List<MobileItem> ml = new List<MobileItem>();

            DataTable dt = DataLink.PropertyData.GetHousePortionMobile();
        ...
        [Route("api/house_portion/{id}")]
        public MobileHousePortion Get(int ID)
        {

            DataTable dt = DataLink.PropertyData.GetHousePortionByIDMobile(ID);
            MobileHousePortion mhp = new MobileHousePortion(int.Parse(dt.Rows[0][0].ToString()), (dt.Rows[0][1]).ToString(), (dt.Rows[0][2].ToString()) + ", " + (dt.Rows[0][3].ToString()), int.Parse(dt.Rows[0][4].ToString()), (dt.Rows[0][5].ToString()), long.Parse(dt.Rows[0][6].ToString()), (dt.Rows[0][7].ToString()), int.Parse(dt.Rows[0][8].ToString()), int.Parse(dt.Rows[0][9].ToString()), (dt.Rows[0][10].ToString()), (dt.Rows[0][11].ToString()), (dt.Rows[0][12].ToString()), (dt.Rows[0][13].ToString()));
            return mhp;
        }
